...
{
	public NatImplementation1Implementation(Variables var,List<Class<? extends ResourceTemplate>> resourceTemplates)
	{
		super(var,resourceTemplates);
		resources.add(new NatImplementation1NatInfrastructureResourceResourceRequirement(var));
	...
	public Double getQoS(Collection<Resource> resourcesUsed)
	{
		...
		try{
			return (resourcesUsed.stream().filter(x -> x.getClass().equals(NatInfrastructureResource.class)).collect(Collectors.toCollection(ArrayList::new)) ).size()*1.0/hosts.size()*1.0;
		}catch(Exception ex){
			return Double.NaN;
		}
	}
