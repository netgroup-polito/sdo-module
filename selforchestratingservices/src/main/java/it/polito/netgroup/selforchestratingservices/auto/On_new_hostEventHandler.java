...
public class On_new_hostEventHandler 
{
	public static boolean fire(Variables var,String hostIp, HostNat hostNat) throws Exception
	{
		...
		NatInfrastructureResource nat = hostNat.getNat();
		hosts.add(host);
		hostNat.getNat().getInfrastructure().addFlowRule(new FlowRule("LB_" + host.getMacAddress().getValue() + counter ,"",2,new Match(lbport,host.getMacAddress().getValue(),lbmac),new Action("vnf:" + nat.getId() + ":User:1" )));
