...
#Dependencies : apt-get install libguestmount (or libguestfs-tools) jq
...
guestmount -a $IMAGE_PATH -m /dev/sda1 mntpoint$RANDOM_SUFFIX
if [ $? -ne 0 ]; then
...
if [ $? -eq 0 ]; then
	for file in $(jq -c ".[]" files.json); do
		...
		if [ $? -ne 0 ]; then
			...
			wget $CONFIG_SERVER/config/files/$FUNCTIONAL_CAPABILITY/$FILENAME -O $FILENAME
